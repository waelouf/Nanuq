...
public class DeleteSet : EndpointWithoutRequest<bool>
{
	private IRedisManagerRepository redisManager;
	private IRedisRepository redisRepository;
	private ICredentialRepository credentialRepository;

	public DeleteSet(IRedisManagerRepository redisManager, IRedisRepository redisRepository, ICredentialRepository credentialRepository)
	{
	...
	{
		Delete("/redis/set/{server}/{database}/{key}");
		AllowAnonymous();
	...
	public override async Task HandleAsync(CancellationToken ct)
	{
		...
		var deleted = await redisManager.DeleteSetAsync(server!, database, key!, credential);
