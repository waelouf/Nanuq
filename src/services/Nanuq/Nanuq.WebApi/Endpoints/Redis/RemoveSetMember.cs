...
public class RemoveSetMember : EndpointWithoutRequest<bool>
{
	private IRedisManagerRepository redisManager;
	private IRedisRepository redisRepository;
	private ICredentialRepository credentialRepository;

	public RemoveSetMember(IRedisManagerRepository redisManager, IRedisRepository redisRepository, ICredentialRepository credentialRepository)
	{
	...
	{
		Delete("/redis/set/{server}/{database}/{key}/member/{value}");
		AllowAnonymous();
	...
	public override async Task HandleAsync(CancellationToken ct)
	{
		...
		var deleted = await redisManager.RemoveSetMemberAsync(server!, database, key!, value!, credential);
