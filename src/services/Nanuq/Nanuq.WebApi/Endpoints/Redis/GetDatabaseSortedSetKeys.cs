...
public class GetDatabaseSortedSetKeys : EndpointWithoutRequest<Dictionary<string, long>>
{
	private IRedisManagerRepository redisManager;
	private IRedisRepository redisRepository;
	private ICredentialRepository credentialRepository;

	public GetDatabaseSortedSetKeys(IRedisManagerRepository redisManager, IRedisRepository redisRepository, ICredentialRepository credentialRepository)
	{
	...
	{
		Get("/redis/sortedset/{server}/{database}");
		AllowAnonymous();
	...
	public override async Task HandleAsync(CancellationToken ct)
	{
		...
		var keys = await redisManager.GetAllDatabaseSortedSetKeys(server!, database, credential);
