...
public class GetSortedSetCount : EndpointWithoutRequest<long>
{
	private IRedisManagerRepository redisManager;
	private IRedisRepository redisRepository;
	private ICredentialRepository credentialRepository;

	public GetSortedSetCount(IRedisManagerRepository redisManager, IRedisRepository redisRepository, ICredentialRepository credentialRepository)
	{
	...
	{
		Get("/redis/sortedset/{server}/{database}/{key}/count");
		AllowAnonymous();
	...
	public override async Task HandleAsync(CancellationToken ct)
	{
		...
		var count = await redisManager.GetSortedSetCountAsync(server!, database, key!, credential);
