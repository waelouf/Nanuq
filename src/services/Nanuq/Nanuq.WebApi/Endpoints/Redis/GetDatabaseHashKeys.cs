...
public class GetDatabaseHashKeys : EndpointWithoutRequest<Dictionary<string, long>>
{
	private IRedisManagerRepository redisManager;
	private IRedisRepository redisRepository;
	private ICredentialRepository credentialRepository;

	public GetDatabaseHashKeys(IRedisManagerRepository redisManager, IRedisRepository redisRepository, ICredentialRepository credentialRepository)
	{
	...
	{
		Get("/redis/hash/{server}/{database}");
		AllowAnonymous();
	...
	public override async Task HandleAsync(CancellationToken ct)
	{
		...
		var keys = await redisManager.GetAllDatabaseHashKeys(server!, database, credential);
