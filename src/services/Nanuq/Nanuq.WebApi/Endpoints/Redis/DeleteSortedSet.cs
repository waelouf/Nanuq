...
public class DeleteSortedSet : EndpointWithoutRequest<bool>
{
	private IRedisManagerRepository redisManager;
	private IRedisRepository redisRepository;
	private ICredentialRepository credentialRepository;

	public DeleteSortedSet(IRedisManagerRepository redisManager, IRedisRepository redisRepository, ICredentialRepository credentialRepository)
	{
	...
	{
		Delete("/redis/sortedset/{server}/{database}/{key}");
		AllowAnonymous();
	...
	public override async Task HandleAsync(CancellationToken ct)
	{
		...
		var deleted = await redisManager.DeleteSortedSetAsync(server!, database, key!, credential);
