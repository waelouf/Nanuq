...
public class GetDatabaseStreamKeys : EndpointWithoutRequest<Dictionary<string, long>>
{
	private IRedisManagerRepository redisManager;
	private IRedisRepository redisRepository;
	private ICredentialRepository credentialRepository;

	public GetDatabaseStreamKeys(IRedisManagerRepository redisManager, IRedisRepository redisRepository, ICredentialRepository credentialRepository)
	{
	...
	{
		Get("/redis/stream/{server}/{database}");
		AllowAnonymous();
	...
	public override async Task HandleAsync(CancellationToken ct)
	{
		...
		var keys = await redisManager.GetAllDatabaseStreamKeys(server!, database, credential);
