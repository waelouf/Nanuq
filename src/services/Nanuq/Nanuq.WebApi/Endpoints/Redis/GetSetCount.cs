...
public class GetSetCount : EndpointWithoutRequest<long>
{
	private IRedisManagerRepository redisManager;
	private IRedisRepository redisRepository;
	private ICredentialRepository credentialRepository;

	public GetSetCount(IRedisManagerRepository redisManager, IRedisRepository redisRepository, ICredentialRepository credentialRepository)
	{
	...
	{
		Get("/redis/set/{server}/{database}/{key}/count");
		AllowAnonymous();
	...
	public override async Task HandleAsync(CancellationToken ct)
	{
		...
		var count = await redisManager.GetSetCountAsync(server!, database, key!, credential);
