...
public class GetHashAllFields : EndpointWithoutRequest<Dictionary<string, string>>
{
	private IRedisManagerRepository redisManager;
	private IRedisRepository redisRepository;
	private ICredentialRepository credentialRepository;

	public GetHashAllFields(IRedisManagerRepository redisManager, IRedisRepository redisRepository, ICredentialRepository credentialRepository)
	{
	...
	{
		Get("/redis/hash/{server}/{database}/{key}");
		AllowAnonymous();
	...
	public override async Task HandleAsync(CancellationToken ct)
	{
		...
		var fields = await redisManager.GetHashAllFieldsAsync(server!, database, key!, credential);
		await Send.OkAsync(fields, ct);
	}
}
