...
public class GetSetMembers : EndpointWithoutRequest<List<string>>
{
	private IRedisManagerRepository redisManager;
	private IRedisRepository redisRepository;
	private ICredentialRepository credentialRepository;

	public GetSetMembers(IRedisManagerRepository redisManager, IRedisRepository redisRepository, ICredentialRepository credentialRepository)
	{
	...
	{
		Get("/redis/set/{server}/{database}/{key}");
		AllowAnonymous();
	...
	public override async Task HandleAsync(CancellationToken ct)
	{
		...
		var members = await redisManager.GetSetMembersAsync(server!, database, key!, credential);
