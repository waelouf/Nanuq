...
public class GetStreamLength : EndpointWithoutRequest<long>
{
	private IRedisManagerRepository redisManager;
	private IRedisRepository redisRepository;
	private ICredentialRepository credentialRepository;

	public GetStreamLength(IRedisManagerRepository redisManager, IRedisRepository redisRepository, ICredentialRepository credentialRepository)
	{
	...
	{
		Get("/redis/stream/{server}/{database}/{key}/length");
		AllowAnonymous();
	...
	public override async Task HandleAsync(CancellationToken ct)
	{
		...
		var length = await redisManager.GetStreamLengthAsync(server!, database, key!, credential);
