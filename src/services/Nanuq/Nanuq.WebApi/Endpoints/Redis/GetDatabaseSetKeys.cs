...
public class GetDatabaseSetKeys : EndpointWithoutRequest<Dictionary<string, long>>
{
	private IRedisManagerRepository redisManager;
	private IRedisRepository redisRepository;
	private ICredentialRepository credentialRepository;

	public GetDatabaseSetKeys(IRedisManagerRepository redisManager, IRedisRepository redisRepository, ICredentialRepository credentialRepository)
	{
	...
	public override async Task HandleAsync(CancellationToken ct)
	{
		...
		var keys = await redisManager.GetAllDatabaseSetKeys(server!, database, credential);
