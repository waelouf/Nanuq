...
public class CreateQueue : Endpoint<CreateQueueRequest>
{
    ...
    public override async Task HandleAsync(CreateQueueRequest req, CancellationToken ct)
    {
        ...
        var details = System.Text.Json.JsonSerializer.Serialize(new
        {
            ...
            maxDeliveryCount = req.MaxDeliveryCount
        });

        await auditLog.Audit(
            ActivityTypeEnum.AddAzureServiceBusQueue,
            $"Azure Service Bus queue '{req.QueueName}' created",
            details
        );
