...
public class DeleteExchange : EndpointWithoutRequest<bool>
{
	private IRabbitMQManagerRepository rabbitMQManager;
	private IRabbitMqRepository rabbitMqRepository;
	private ICredentialRepository credentialRepository;

	public DeleteExchange(IRabbitMQManagerRepository rabbitMQManager,
	...
	{
		Delete("/rabbitmq/exchange/{serverUrl}/{name}");
		AllowAnonymous();
	...
	public override async Task HandleAsync(CancellationToken ct)
	{
		...
		var request = new DeleteExchangeRequest(serverUrl!, name!);
		var result = await rabbitMQManager.DeleteExchangeAsync(request, credential);
